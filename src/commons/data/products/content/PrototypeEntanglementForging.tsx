    ...
    overview: {
      primaryDescription: (
        <>
          ...
          This module allows a user to simulate chemical and physical systems using a Variational Quantum Eigensolver (VQE) enhanced by Entanglement Forging 
          {' '}<a
          ...
          Entanglement Forging doubles the size of the system that can be exactly simulated on a fixed set of quantum bits.
          </Typography>
          ...
          <Typography as='p' marginBottom='1rem'>
          Entanglement Forging leverages near-term, heuristic algorithms, such as VQE, to provide an estimate of the 2n-qubit expectation value. It does so by assuming a parameterized ansatz for the wavefunction of each sub-system. (Note that the parameters of this ansatz describe the unitaries U and V in the Schmidt decomposition.) After the expectation value M has been decomposed into sub-experiments, the procedure is as follows:
           ...
           <Typography as='p' marginBottom='0'>
            2. Combine the expectation values for the sub-experiments with the weights wa,b and the Schmidt parameters λn to obtain an estimate for M.

          </Typography>
           <Typography as='p' marginBottom='0'>
            3. Send the estimate of M, along with λn and the variational parameters {'{θ}'} describing U and V, to a classical optimizer.

          </Typography>
           <Typography as='p' marginBottom='0'>
            4. Use the classical optimizer to further minimize M and provide a new set for the variational parameters {'{θ}'} and Schmidt coefficients λn.
           ...
           <Typography as='p' marginBottom='1rem'>
           Note that if M is the expectation value of the system's Hamiltonian, then it is possible to separate the optimization over the variational parameters {'{θ}'} and the Schmidt coefficients λn. In particular, the Schmidt coefficients can be optimized after step 2, and separately from the variational parameters.

          </Typography>
           <Typography as='p' marginBottom='1rem'>
           Further, an easy way to reduce the number of sub-experiments necessary is by truncating the Schmidt decomposition of {'|ψ>'} to include only some number of the bitstring states {'|bn>'}. However, doing so will generally lead to less accuracy in the estimation of the expectation value.
    ...
    instructions: {
      description:(
        <>
          ...
          <Typography as='p' marginBottom='1rem'>
          This prototype depends on the PySCF package, which does not support Windows; therefore, Windows users will not be able to install and use this software. Advanced Windows users may wish to attempt to install PySCF using Ubuntu via the Windows Subsystem for Linux. We are exploring the possibility of providing Docker support for this prototype so it can be used within Docker Desktop, including on Windows.
          </Typography>
          <Typography as='p' marginBottom='1rem'>
        ...
        <UnorderedList nested>
          ...
          <Typography as='p' marginLeft = '1.5rem' marginTop='.5rem' marginBottom='0rem'>
...
2. Users may now run the entanglement forging demo notebooks on their local machine or use the entanglement-forging package in their own software.
          ...
          <Typography as='p' marginBottom='1rem'>
0. Make sure you have git and pip (and optionally Miniconda) installed.
...
1. From the terminal, clone repository:
          ...
          Alternatively, instead of cloning the original repository, you may choose to clone your personal fork. You can do so by using the appropriate URL and adding the original repo to the list of remotes (here under the name upstream). This will be requiered for contribution unless you are granted write permissions for the original repository.
